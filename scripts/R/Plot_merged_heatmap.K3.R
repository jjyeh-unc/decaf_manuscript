Plot_merged_heatmap.K3 <- function(rDataName, dataSet, cafSubtype, sampSub, sampSubLabel = "") {
  
  if(sampSubLabel == "") {
    pdf(paste("../heatmap/",rDataName,".K3.pdf",sep=""))
  } else {
    pdf(paste("../heatmap/",rDataName,".",sampSubLabel,".K3.pdf",sep=""))
  }
  ...
  ColSideColors <- cbind(ColSideColors0[sampSub,],
                         data.frame(SCISSORS_CAF_top10 = cafSubtype$SCISSORS_CAF_K2_top10.unscaled$ColSideColors,
                                    SCISSORS_CAF_top25 = cafSubtype$SCISSORS_CAF_K2_top25.unscaled$ColSideColors,
                                    stringsAsFactors = FALSE))
