...
study_subtype_file = list.files(path = "../../data/public_PDAC/",pattern = "*caf_subtype.rds")
...
for(i in 1:length(study_subtype_names)){
    ...
    sub = readRDS(sprintf("../../data/public_PDAC/%s.caf_subtype.rds", study_subtype_names[i] )) 
    ...
    dat = try(readRDS(sprintf("../../data/public_PDAC/%s.rds", study_subtype_names[i])))
...
source("../R/DeCAF/classifier_training_functions.R")
