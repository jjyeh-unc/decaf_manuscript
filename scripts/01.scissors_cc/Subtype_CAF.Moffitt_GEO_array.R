...
############################## Subtyping ##############################
if(FALSE) {
load("../data/Moffitt_GEO_array_plus.RData")
dataSet <- Moffitt_GEO_array_plus
rDataName <- "Moffitt_GEO_array"
saveRDS(dataSet, file = paste("../data_2023/",rDataName,".rds",sep=""))
}
rDataName <- "Moffitt_GEO_array"
dataSet <- readRDS("../data_DeCAF/Moffitt_GEO_array.rds")
sampSub <- which(dataSet$sampInfo$specimen_type %in% c("Primary"))
cafSubtype <- list()
...
tmpK <- 7
dataSet <- Call_consensus(dataSet, tmpSchema, sampSub, 2, tmpK, tmpRowScale, TRUE, FALSE, "km","euclidean", "R-4.X.X")
tmpSubtypeLab <- c("iCAF","myCAF","Mixed","myCAF","iCAF","Mixed.iCAF","Mixed.myCAF")
dataSet <- Relabel_subtype(dataSet, tmpSubtypeLab, tmpSchema, tmpK, tmpRowScale)
...
tmpK <- 2
dataSet <- Call_consensus(dataSet, tmpSchema, sampSub, 2, tmpK, tmpRowScale, TRUE, FALSE, "km","euclidean", "R-4.X.X")
tmpSubtypeLab <- c("Normal","Activated")
dataSet <- Relabel_subtype(dataSet, tmpSubtypeLab, tmpSchema, tmpK, tmpRowScale)
cafSubtype <- Save_cafSubtype(cafSubtype, dataSet, tmpSchema, tmpK, tmpRowScale)
