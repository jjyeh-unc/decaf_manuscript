...
############################## Subtyping ##############################
if(FALSE) {
load("../data/Linehan_Seq_plus.RData")
dataSet <- Linehan_Seq_plus
rDataName <- "Linehan"
saveRDS(dataSet, file = paste("../data_2023/",rDataName,".rds",sep=""))
}
rDataName <- "Linehan"
dataSet <- readRDS("../data_DeCAF/Linehan.rds")
sampSub <- which(!is.na(dataSet$sampInfo$Treatment))
cafSubtype <- list()
...
tmpK <- 5
dataSet <- Call_consensus(dataSet, tmpSchema, sampSub, 2, tmpK, tmpRowScale, TRUE, FALSE, "km","euclidean", "R-4.X.X")
tmpSubtypeLab <- c("Mixed.myCAF","iCAF","myCAF","Mixed","Mixed")
dataSet <- Relabel_subtype(dataSet, tmpSubtypeLab, tmpSchema, tmpK, tmpRowScale)
