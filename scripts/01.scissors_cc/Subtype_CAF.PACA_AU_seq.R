...
############################## Subtyping ##############################
if(FALSE) {
load("../data/PACA_AU_seq_plus.RData")
dataSet <- PACA_AU_seq_plus
rDataName <- "PACA_AU_seq"
saveRDS(dataSet, file = paste("../data_2023/",rDataName,".rds",sep=""))
}
rDataName <- "PACA_AU_seq"
dataSet <- readRDS("../data_DeCAF/PACA_AU_seq.rds")
...
tmpK <- 8
dataSet <- Call_consensus(dataSet, tmpSchema, sampSub, 2, tmpK, tmpRowScale, TRUE, FALSE, "km","euclidean", "R-4.X.X")
tmpSubtypeLab <- c("Mixed.myCAF","Mixed.iCAF","Mixed.myCAF","Mixed","Mixed","Mixed.iCAF","Mixed","Absent")
dataSet <- Relabel_subtype(dataSet, tmpSubtypeLab, tmpSchema, tmpK, tmpRowScale)
